...
if (typeof showNotification === 'undefined') {
  ...
  if (ws && ws.readyState === WebSocket.OPEN) {
    ws.send(JSON.stringify({ type: 'auth', action: 'register', username, password }));
  }
...
function saveGameData() {
  ...
  ws.send(JSON.stringify({
    ...
    data: {
      ...
      currentSkin: game.currentSkin,
      achievements: game.achievements,
      quests: game.quests.map(q => ({ id: q.id, completed: q.completed })),
...
function logout() {
  ...
  game.totalCoins = 0;
  game.clicks = 0;
  game.skins = { normal: true };
  game.currentSkin = 'normal';
  pendingBoxes = [];
