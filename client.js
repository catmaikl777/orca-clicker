...
function showCatdropReward(reward) {
  ...
  rewardModal.innerHTML = `
    <div class="catdrop-reward-content" style="box-shadow: ${rarityGlow[reward.rarity] || rarityGlow.common}">
      ...
      <button class="catdrop-reward-btn" onclick="closeCatdropReward()" style="pointer-events:auto;">Закрыть</button>
    </div>
  `;
...
window.forceCloseAllModals = function() {
  ...
  if (catdropModal) {
    ...
    catdropModal.style.pointerEvents = 'none';
  }
  
  console.log('✅ Все modal-ы закрыты');
};

// Функция для отладки: показывает что блокирует клики
window.debugCheckClickBlockers = function() {
  console.log('🔍 debugCheckClickBlockers: проверяю что блокирует клики...');
  ...
  allElements.forEach(el => {
    ...
    if (zIndex > 1000 && display !== 'none' && pointerEvents !== 'none') {
      blockers.push({
        tag: el.tagName,
        id: el.id || 'no-id',
        class: el.className,
  ...
  console.log('🔍 Найдены элементы с z-index > 1000:', blockers);
  
  // Проверяем есть ли overlay блокирующие клики
  const overlay = document.elementFromPoint(window.innerWidth/2, window.innerHeight/2);
...
document.addEventListener('click', function(e) {
  const now = Date.now();
  if (e.clientX < 50 && e.clientY < 50) {  // Левый верхний угол
    if (now - lastTap < 500) {
      tapCount++;
      if (tapCount >= 3) {
        tapCount = 0;
        console.log('🔥 Трижды тап в углу! Открываю отладку...');
        window.debugCheckClickBlockers();
...
function showCatdropWaitingScreen() {
  ...
  // ВКЛЮЧАЕМ pointer-events для cutscene
  setTimeout(() => {
    cutscene.classList.add('active');
    console.log('✅ Catdrop cutscene активирован');
  }, 10);
...
function stopCatdropWaitingScreen() {
  if (activeBoxCutscene) {
    activeBoxCutscene.classList.remove('active');
    activeBoxCutscene.remove();
    activeBoxCutscene = null;
  }
